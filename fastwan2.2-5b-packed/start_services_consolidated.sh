...
# FastWAN 2.2-5B Consolidated ComfyUI Startup Script
...
TS_UP_ARGS=("--hostname=${RUNPOD_POD_HOSTNAME:-fastwan-comfy-pod}" "--accept-dns=false")
...
echo "[services] Setting up ComfyUI with FastWAN 2.2-5B..."
...
echo "[services] RTX 4090 optimizations enabled:"
echo "[services] - TORCH_INDUCTOR_FORCE_DISABLE_FP8=${TORCH_INDUCTOR_FORCE_DISABLE_FP8}"
echo "[services] - CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
echo "[services] - PYTORCH_CUDA_ALLOC_CONF=${PYTORCH_CUDA_ALLOC_CONF}"
...
# Verify ComfyUI installation and models
if [ -f "${COMFY_DIR}/main.py" ]; then
  echo "[services] Found ComfyUI installation at ${COMFY_DIR}."
  cd "${COMFY_DIR}"

  # Verify FastWAN models are present
  echo "[services] Verifying FastWAN 2.2-5B models..."
  MODELS_MISSING=false
  
  if [ ! -f "models/diffusion_models/wan2.2_ti2v_5B_fp16.safetensors" ]; then
    echo "[services] ERROR: FastWAN diffusion model not found!"
    MODELS_MISSING=true
  fi
  
  if [ ! -f "models/text_encoders/umt5_xxl_fp8_e4m3fn_scaled.safetensors" ]; then
    echo "[services] ERROR: FastWAN text encoder not found!"
    MODELS_MISSING=true
  fi
  
  if [ ! -f "models/vae/wan2.2_vae.safetensors" ]; then
    echo "[services] ERROR: FastWAN VAE not found!"
    MODELS_MISSING=true
  fi
  
  if [ ! -f "models/loras/Wan2_2_5B_FastWanFullAttn_lora_rank_128_bf16.safetensors" ]; then
    echo "[services] ERROR: FastWAN LoRA not found!"
    MODELS_MISSING=true
  fi
  
  if [ ! -d "custom_nodes/fastwan-moviegen" ]; then
    echo "[services] ERROR: FastWAN custom node not found!"
    MODELS_MISSING=true
  fi
  
  if [ "$MODELS_MISSING" = true ]; then
    echo "[services] CRITICAL: Some FastWAN models are missing. Please rebuild the Docker image."
    exit 1
  fi
  
  echo "[services] ✓ All FastWAN 2.2-5B models verified successfully!"

  # Install any additional custom node dependencies
  install_node_deps "${COMFY_DIR}/custom_nodes" "FastWAN Setup"

  echo "[services] Launching ComfyUI with FastWAN 2.2-5B: ${COMFY_LAUNCH_ARGS}"
  ...
  echo "[services] CRITICAL ERROR: ComfyUI not found at ${COMFY_DIR}!"
  echo "[services] This should not happen in the consolidated image."
  exit 1
fi

deactivate
echo "[services] ComfyUI with FastWAN 2.2-5B setup complete."
...
echo "[services] Starting Caddy reverse proxy..."
...
echo "[services] FastWAN 2.2-5B ComfyUI startup complete!"
echo "[services] ComfyUI available at: http://localhost:8188"
echo "[services] Waiting on: ${PIDS_TO_WAIT[*]:-none}"
